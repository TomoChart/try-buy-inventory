...
export interface GalaxyTryRecord {
...
const normalizeBackendRow = (r: any): GalaxyTryRecord => {
...
const mapUiToImportRow = (r: GalaxyTryRecord): ImportRow => {
...
async function importToBackend(country: string, rows: GalaxyTryRecord[]) {
...
function GalaxyTryPage() {
  ...
  const [data, setData] = useState<GalaxyTryRecord[]>([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const mapped: GalaxyTryRecord[] = Array.isArray(raw) ? raw.map(normalizeBackendRow) : [];
        setData(mapped);
      } catch (e) {
  ...
  const col = (
    key: keyof GalaxyTryRecord,
    width: string,
    cell?: any,
    title?: string,
    noWrap?: boolean
  ): ColumnDef<GalaxyTryRecord> => ({
    accessorKey: key,
    header: ({ column }) => (
  ...
  const columns = useMemo<ColumnDef<GalaxyTryRecord>[]>(
    () => [
      {
        id: "select",
        header: ({ table }) => (
  ...
  const table = useReactTable({
    ...
    meta: {
      async updateData(rowIndex, columnId, value) {
        ...
        setData((old) => {
          const next = [...old];
          next[rowIndex] = { ...next[rowIndex], [columnId]: value } as GalaxyTryRecord;
          return next;
        });
        ...
        try {
          ...
          await patchGalaxyTryField(country, submissionId, columnId, value);
          toast.success("Saved");
        } catch (e: any) {
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      const imported: GalaxyTryRecord[] = rows.map((r) => ({
      ...
      setData((prev) => {
        const map = new Map<string, GalaxyTryRecord>();
...
function ColumnHeader({
  ...
  column: Column<GalaxyTryRecord, unknown>;
  title: string;
}) {
...
async function patchGalaxyTryField(countryCode: string, submissionId: string, field: string, value: unknown) {
  const token = getToken();
  if (!token) throw new Error("No token");
