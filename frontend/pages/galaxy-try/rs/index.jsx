...
function GalaxyTryRS() {
  ...
  return (
    <div className="p-6">
      ...
      <h1 className="text-xl font-bold mb-4">Galaxy Try — RS</h1>
          ...
          <tbody>
            {rows.map((r,i) => (
              <tr key={r.submission_id || i} className="border-t">
                <td className="p-2">{r.first_name || r["Ime"] || "-"}</td>
                <td className="p-2">{r.last_name  || r["Prezime"] || "-"}</td>
                <td className="p-2">{r.email      || r["E-mail"] || "-"}</td>
                <td className="p-2">{r.city       || r["Grad"] || "-"}</td>
...
export default withAuth(GalaxyTryRS, { roles: ["COUNTRY_ADMIN","SUPERADMIN"] });
