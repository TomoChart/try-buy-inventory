...
function GalaxyTryRSPage() {
  ...
  async function saveEdit(id) {
    ...
    const r = await fetch(`${API}/admin/galaxy-try/rs/${id}`, {
      method: "PATCH",
  ...
  async function handleDelete(submissionId) {
    try {
      ...
      const res = await fetch(`${API}/admin/galaxy-try/rs/${encodeURIComponent(submissionId)}`, {
        method: 'DELETE',
  ...
  async function handleDeleteSelected() {
    ...
    for (const id of selected) {
      await fetch(`${API}/admin/galaxy-try/rs/${encodeURIComponent(id)}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${getToken()}` },
      });
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <h1 className="text-xl font-bold">Galaxy Try — RS</h1>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded shadow p-4 w-[720px] max-w-[95vw]">
            <h3 className="font-semibold text-lg mb-3">Add new — RS</h3>
            <AddForm
...
async function handleImportGalaxyCsv(e) {
  ...
  const endpoint = `${API}/admin/galaxy-try/rs/import?mode=upsert`;
...
function EditForm({ initial, onCancel, onSaved }) {
  ...
  async function save() {
    try {
      setSaving(true);
      const res = await fetch(`${API}/admin/galaxy-try/rs/${encodeURIComponent(initial.submission_id)}`, {
        method: "PATCH",
        headers: {
  ...
  async function save() {
    try {
      setSaving(true);
      const res = await fetch(`${API}/admin/galaxy-try/rs`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${getToken()}`
...
export default withAuth(GalaxyTryRSPage, { roles: ["COUNTRY_ADMIN", "SUPERADMIN"] });
