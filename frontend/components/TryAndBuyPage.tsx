...
const calcDaysLeft = (handover: string | null, returned?: boolean): number | "" => {
  if (!handover || returned) return "";
  const handoverDate = new Date(handover);
  const diff = Math.floor(
    (new Date().getTime() - handoverDate.getTime()) / (1000 * 60 * 60 * 24)
  );
  return 14 - diff;
};
  ...
  const columns = useMemo<ColumnDef<TryBuyRecord>[]>(
    () => [
      ...
      },
      {
        ...
        header: "days_left",
        cell: ({ row }) => {
          const v = calcDaysLeft(row.original.handover_at, row.original.returned);
      ...
      { accessorKey: "note", header: "note", cell: EditableCell },
      {
        ...
        cell: ({ row, table }) => (
          <input
            type="checkbox"
            checked={row.original.returned || false}
            onChange={(e) =>
              table.options.meta?.updateData(row.index, "returned", e.target.checked)
            }
          />
        ),
      },
      { accessorKey: "user_feedback", header: "User feedback", cell: EditableCell },
    ],
    []
  );
        ...
        <table className="min-w-full text-sm">
            ...
            {table.getHeaderGroups().map((hg) => (
              <tr key={hg.id}>
                {hg.headers.map((header) => (
                  <th key={header.id} className="p-2 text-left relative">
          ...
          <tbody>
            {table.getRowModel().rows.map((row) => {
              const days = calcDaysLeft(row.original.handover_at, row.original.returned);
              return (
                <tr
                  key={row.id}
                  className={`${
...
function SelectCell(options: string[]) {
  return function SelectCellComponent({ getValue, row, column, table }: any) {
    ...
    return (
      <select className="p-1" value={value} onChange={onChange}>
        {options.map((o) => (
...
function HeaderMenu({ header }: { header: any }) {
  ...
  return (
    <div>
      ...
      {open && (
        <div className="absolute bg-white border shadow p-2 mt-1">
          <div className="cursor-pointer" onClick={() => { column.toggleSorting(false); setOpen(false); }}>
            Sort A-Z
          </div>
          <div className="cursor-pointer" onClick={() => { column.toggleSorting(true); setOpen(false); }}>
            Sort Z-A
          </div>
          <input
            className="p-1 mt-1"
